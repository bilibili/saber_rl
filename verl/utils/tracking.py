    ...
    def __init__(self, project_name, experiment_name, default_backend: str | list[str] = "console", config=None):
        ...
        if 'tensorboard' in default_backend:
            try:
                ...
                bvac_job_id = os.getenv('JOB_ID', '0')
                summary_root = config['trainer'].get('summary_dir', '/summary_dir')
                if summary_root is None:
                    summary_root = "/summary_dir"
                summary_dir = os.path.join(summary_root, bvac_job_id)
                tb_writer = SummaryWriter(summary_dir, filename_suffix=bvac_job_id)
                self.logger['tensorboard'] = _TensorboardLoggingAdapter(tb_writer)
            except ImportError:
                warnings.warn("TensorBoard requires torch.utils.tensorboard. Please install with: pip install tensorboard")
...
class _TensorboardLoggingAdapter:
    """适配器类将数据转换为TensorBoard需要的格式"""
    def __init__(self, writer):
        self.writer = writer

    def log(self, data, step):
        if not isinstance(data, dict):
            raise TypeError("TensorBoard logger requires data to be a dictionary")
            
        for key, value in data.items():
            try:
                if isinstance(value, (int, float)):
                    self.writer.add_scalar(key, value, step)
                # 可以扩展支持其他数据类型（如图像、直方图等）
                # elif isinstance(value, torch.Tensor):
                #     self.writer.add_image(key, value, step)
            except Exception as e:
                warnings.warn(f"Failed to log {key} to TensorBoard: {str(e)}")
        self.writer.flush()
