...
from verl.bl_utils.async_saver import gen_remote_path
from verl.bl_utils.future_collector import get_future_collector, FutureWrapper
    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        with warnings.catch_warnings():
            ...
            with get_fsdp_state_ctx(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
                ...
                node_rank = os.environ["PET_NODE_RANK"]
                resource_key = f"pet_node_rank_{node_rank}"
                print(f"[rank-{self.rank}]: uploading ckpts with node {resource_key=}")
                ...
                if self.should_save_model:
                    ...
                    f1 = (
                        ray.remote(hybrid_copy)
                        .options(resources={resource_key: 1})
                        .remote(
                            model_path,
                            gen_remote_path(model_path, local_root=self.config.trainer.default_local_dir)
                        )
                    )
                    ray.get(collector.add_future.remote("ckpt_uploading", FutureWrapper(f1)))
                ...
                if self.should_save_optimizer:
                    ...
                    f2 = (
                        ...
                        .remote(
                            optim_path,
                            gen_remote_path(optim_path, local_root=self.config.trainer.default_local_dir),
                            delete_after_upload=self.config.trainer.delete_after_upload
                        )
                    )
                    ray.get(collector.add_future.remote("ckpt_uploading", FutureWrapper(f2)))
                ...
                if self.should_save_extra:
                    ...
                    f3 = (
                        ...
                        .remote(
                            extra_path,
                            gen_remote_path(extra_path, local_root=self.config.trainer.default_local_dir),
                            delete_after_upload=self.config.trainer.delete_after_upload
                        )
                    )
                    ray.get(collector.add_future.remote("ckpt_uploading", FutureWrapper(f3)))
        ...
        if self.rank == 0:
            ...
            # upload huggingface dir and fsdp_config.json
            f4 = (
                ray.remote(hybrid_copy)
                .options(resources={resource_key: 1})
                .remote(
                    hf_config_tokenizer_path,
                    gen_remote_path(hf_config_tokenizer_path, local_root=self.config.trainer.default_local_dir)
                )
            )
            ray.get(collector.add_future.remote("ckpt_uploading", FutureWrapper(f4)))
            f5 = (
                ray.remote(hybrid_copy)
                .options(resources={resource_key: 1})
                .remote(
                    fsdp_config_path,
                    gen_remote_path(fsdp_config_path, local_root=self.config.trainer.default_local_dir)
                )
            )
            ray.get(collector.add_future.remote("ckpt_uploading", FutureWrapper(f5)))
