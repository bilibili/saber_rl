...
from verl.bl_utils.global_config import update_full_config
from verl.bl_utils.future_collector import get_future_collector
from verl.bl_utils.global_config import get_config, set_config
...
class ResourcePoolManager:
    ...
    def _check_resource_available(self):
        ...
        node_available_gpus = {node: node_info.get("GPU", 0) if "GPU" in node_info else node_info.get("NPU", 0) for
...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False,
                      norm_adv_by_std_in_grpo=True, **kwargs):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        if multi_turn:
            ...
            grpo_calculation_mask = data.batch["loss_mask"][
                :, -response_length:]  # This mask is the one intended for GRPO
...
class RayPPOTrainer:
    ...
    def _validate_config(self):
        ...
        if config.actor_rollout_ref.actor.strategy == "megatron":
            ...
            assert n_gpus % (
                        model_parallel_size * config.actor_rollout_ref.actor.megatron.context_parallel_size) == 0, f"n_gpus ({n_gpus}) must be divisible by model_parallel_size ({model_parallel_size}) times context_parallel_size ({config.actor_rollout_ref.actor.megatron.context_parallel_size})"
            megatron_dp = n_gpus // (
    ...
    def _maybe_log_val_generations_to_txt(self, inputs, outputs, scores, reward_extra_infos_dict=None):
        """Log validation samples to a local txt file, with each step saved in a separate file"""

        generations_to_log = self.config.trainer.val_generations_to_log_to_txt
        ...
        # Create tuples of (input, output, score, rm_response) and sort by input text
        if not reward_extra_infos_dict:
            samples = list(zip(inputs, outputs, scores))
        else:
            data_sources = reward_extra_infos_dict["data_source"]
            errors = reward_extra_infos_dict["error"]
            answer_scores = reward_extra_infos_dict["answer_score"]
            format_scores = reward_extra_infos_dict["format_score"]
            length_scores = reward_extra_infos_dict["length_score"]
            count_scores = reward_extra_infos_dict["count_score"]
            sim_scores = reward_extra_infos_dict["sim_score"]
            rm_response_list = reward_extra_infos_dict["rm_response"]
            think_token_counts = reward_extra_infos_dict["think_token_count"]
            answer_token_counts = reward_extra_infos_dict["answer_token_count"]
            llm_equals = reward_extra_infos_dict["llm_equal"]
            if_llm_equals = reward_extra_infos_dict["if_llm_equal"]
            overlong_rewards = reward_extra_infos_dict["overlong_reward"]
            llm_response_list = reward_extra_infos_dict["llm_response"]
            ground_truths = reward_extra_infos_dict["ground_truth"]

            samples = list(zip(inputs,
                               outputs,
                               scores,
                               data_sources,
                               errors,
        ...
        # Define the output file path based on self.global_steps
        output_file = f"/model_dir/validation_generations_step_{self.global_steps}.txt"

        # Write data to the file
        with open(output_file, "w") as f:  # Use "w" mode to create a new file for each step
            if not reward_extra_infos_dict:
                for i, (input_text, output_text, score) in enumerate(samples):
                    # Write sample data
                    f.write("\n" + "=" * 80)
                    f.write(" Processing New Sample ".center(80, '='))
                    f.write(f"\n[Input Text]\n{input_text}\n")
                    f.write(f"\n[Output Text]\n{output_text}\n")
                    f.write(f"\n[Score]: {score}\n")
                    f.write("=" * 80 + "\n")
            else:
                for i, (input_text, output_text, score, data_source,
                        error, answer_score, format_score, length_score, count_score, sim_score,
                        rm_response, think_token_count, answer_token_count,
                        llm_equal, if_llm_equal, overlong_reward, llm_response, ground_truth) in enumerate(samples):
                    # Write sample data
                    f.write("\n" + "=" * 80)
                    f.write(" Processing New Sample ".center(80, '='))
                    f.write(f"\n[Input Text]\n{input_text}\n")
                    ...
                    f.write(f"\n[Sim Score]: {sim_score}\n")
                    f.write(f"\n[Overlong Score]: {overlong_reward}\n")
                    f.write(f"\n[RM Response]: \n{rm_response}\n")
                    f.write(f"\n[LLM Equal]: {llm_equal}\n")
                    f.write(f"\n[If LLM Equal]: {if_llm_equal}\n")
                    f.write(f"\n[LLM Response]: \n{llm_response}\n")
                    f.write(f"\n[Think Token Count]: {think_token_count}\n")
                    f.write(f"\n[Answer Token Count]: {answer_token_count}\n")
                    f.write("=" * 80 + "\n")

        print(f"Validation generations for step {self.global_steps} logged to {output_file}")
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            if "reward_extra_info" in result:
                # reward_extra_info keys:  dict_keys(['answer_score', 'format_score', 'length_score', 'rm_response', 'think_token_count', 'answer_token_count', 'llm_equal', 'if_llm_equal', 'llm_response', 'ground_truth', 'data_source', 'error'])
                print("reward_extra_info keys: ", result["reward_extra_info"].keys())
        ...
        self._maybe_log_val_generations_to_txt(inputs=sample_inputs,
            ...
            for var_name, metric2val in var2metric2val.items():
                ...
                for metric_name, metric_val in metric2val.items():
                    if (var_name == core_var) and any(
                            metric_name.startswith(pfx) for pfx in ["mean", "maj", "best"]) and (
    ...
    def _save_checkpoint(self):
        # clear ckpt uploading stub from driver, to ignore previous uploading status
        collector = get_future_collector()
        ray.get(collector.clear.remote("ckpt_uploading"))
        ...
        dataloader_remote_path = gen_remote_path(dataloader_local_path,
                                                 local_root=self.config.trainer.default_local_dir)
        if dataloader_remote_path:
            ray.remote(hybrid_copy).remote(dataloader_local_path, dataloader_remote_path)
            print(f"Appended ray task to upload dataloader to {dataloader_remote_path}")

        latest_checkpointed_iteration_remote_path = gen_remote_path(local_latest_checkpointed_iteration,
                                                                    local_root=self.config.trainer.default_local_dir)
        if latest_checkpointed_iteration_remote_path:
            ray.remote(hybrid_copy).remote(local_latest_checkpointed_iteration,
                                           latest_checkpointed_iteration_remote_path)
            print(f"Appended ray task to upload dataloader to {latest_checkpointed_iteration_remote_path}")
    ...
    def _balance_batch(self, batch: DataProto, metrics, logging_prefix="global_seqlen"):
        ...
        global_partition_lst = get_seqlen_balanced_partitions(global_seqlen_lst, k_partitions=world_size,
                                                              equal_size=True)
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # collect metrics
                metrics.update(compute_reward_metrics(batch, reward_extra_infos_dict))
