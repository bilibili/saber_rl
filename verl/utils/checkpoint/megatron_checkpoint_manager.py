...
from verl.bl_utils.fs_io import upload_with_rank
...
class MegatronCheckpointManager(BaseCheckpointManager):
    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        if self.should_save_model and (not self.should_save_hf_model):
        ...
        if self.should_save_model and self.should_save_hf_model:
            if self.bridge:
        ...
        if self.rank == 0 and False == self.use_dist_checkpointing:
            # Save tokenizer
        ...
        torch.distributed.barrier()
        if True == self.config.actor.checkpoint.export:
            rtv = upload_with_rank(local_path, self.config.actor.checkpoint.default_local_dir, self.rank, \
                                   use_async=True, delete_after_upload=self.config.actor.checkpoint.delete_after_upload)
        torch.distributed.barrier()
