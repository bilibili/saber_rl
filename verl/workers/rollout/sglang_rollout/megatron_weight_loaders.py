...
def qwen2_megatron_weight_loader(actor_weights: Dict, vllm_model: nn.Module) -> nn.Module:
    ...
    for name, loaded_weight in actor_weights.items():
        ...
        # print(f"Before weight actually load, name = {name}, param.szie = {param.size()}, loaded_weight.size = {loaded_weight.size()}")
        # weight_loader = getattr(param, "weight_loader", default_weight_loader)
        # weight_loader(param, loaded_weight)
        
        # TODO: fixed by index, load sharded param directly, need a better way
